...
public abstract class RolloutContractTest<U extends IRolloutUser> {
	...
	public void activating_and_deactivating_for_user() {
		...
		assertThat("should be activated", rollout.isActive("chat", createNewRolloutUser(1)), is(true));

		rollout.deactivateUser("chat", createNewRolloutUser(1));

		assertThat("should be deactivated", rollout.isActive("chat", createNewRolloutUser(1)), is(false));
	}
